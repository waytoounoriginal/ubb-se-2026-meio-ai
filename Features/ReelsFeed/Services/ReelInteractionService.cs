...
namespace ubb_se_2026_meio_ai.Features.ReelsFeed.Services
{
    ...
    public class ReelInteractionService : IReelInteractionService
    {
        ...
        public async Task ToggleLikeAsync(int userId, int reelId)
        {
            ...
            var existingInteraction = await this.interactionRepository.GetInteractionAsync(userId, reelId);
        ...
        {
            var preferenceExists = await this.preferenceRepository.PreferenceExistsAsync(userId, movieId);

            if (!preferenceExists)
            {
                await this.preferenceRepository.InsertPreferenceAsync(userId, movieId, LikeBoostAmount);
            }
            else
            {
                await this.preferenceRepository.UpdatePreferenceAsync(userId, movieId, LikeBoostAmount);
            }
        }
        ...
        {
            await this.interactionRepository.UpdateViewDataAsync(userId, reelId, watchDurationSec, watchPercentage);
        }
